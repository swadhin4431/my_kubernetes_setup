...
EOF
    ...
    if [[ ! -f /etc/kubernetes/admin.conf ]]; then
        echo "Initializing Kubernetes worker..."
        kubeadm join 192.168.31.120:6443 --token qmuccw.fqbqhhg9all40awz \
        --cri-socket unix:///var/run/crio/crio.sock \
        --discovery-token-ca-cert-hash sha256:8b5c5a61697a1e0b003a936abbe26549634ae273460dd5cf7bac6a84a7341b91 \
        --ignore-preflight-errors=SystemVerification,FileExisting-tc,Firewalld
    else
        echo 'this `hostname` is already added to the kube cluster'
    fi
